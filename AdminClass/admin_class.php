...
Class Action {
	...
	function save_cocu(){
		...
		$chk = $this->db->query("SELECT * FROM cocu_track where cocu_code ='$cocu_code' and id != '$id' ");
		if($chk->num_rows > 0){
			return 2;
			exit;
		}
		if(empty($id)){
			$save = $this->db->query("INSERT INTO cocu_track set $data");
		}else{
			$save = $this->db->query("UPDATE cocu_track set $data where id = $id");
		}
		if($save){
			return 1;
		}
	...
	function delete_cocu(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM cocu_track where id = $id");
		if($delete){
			return 1;
		}
	}
