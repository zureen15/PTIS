<?php
include '../config.php';
$qry = $conn->query("SELECT a.*,concat(s.firstname,' ',s.lastname) as name,s.student_code,concat(c.level,'-',c.section) as class,s.gender FROM results a inner join class c on c.id = a.class_id inner join students s on s.id = a.student_id where a.id = ".$_GET['id'])->fetch_array();
foreach($qry as $k => $v){
	$$k = $v;
}
?>
<div class="container-fluid" id="printable">
	...
	<table class="table table-bordered">
		...
		<tbody>
			<?php 
    			$items=$conn->query("SELECT a.*,s.subject_code,s.subject FROM result_items a inner join subjects s on s.id = a.subject_id where result_id = $id  order by s.subject_code asc");
