          ...
          <div class="col-md-6">
            ...
            <div class="form-group">
                ...
                <select name="student_id" id="student_id" class="form-control select2 select2-sm" required>
                  <option></option> 
                  <?php 
                        $classes = $conn->query("SELECT s.*,concat(c.level,'-',c.section) as class,concat(firstname,' ',lastname) as name FROM students s inner join class c on c.id = s.class_id inner join user_form us on s.class_id = us.class_id order by concat(firstname,' ',lastname) asc ");
