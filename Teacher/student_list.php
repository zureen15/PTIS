		...
		<div class="card-body">
			<table class="table tabe-hover table-bordered" id="table">
				...
				<tbody>
					<?php
					$i = 1;
					$qry = $conn->query("SELECT s.*,concat(c.level,'-',c.section) as class,concat(s.firstname,' ',s.lastname) as name FROM students s inner join class c on c.id = s.class_id inner join user_form us on c.id = us.class_id order by concat(firstname,' ',lastname) asc ");
