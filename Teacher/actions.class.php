...
class Actions{
    ...
    public function list_class(){
        $sql = "SELECT * FROM class c inner join user_form us on c.id = us.class_id order by level asc,section asc ";
        $qry = $this->conn->query($sql);
    ...
    public function list_student(){
        $sql = "SELECT s.*,concat(c.level,'-',c.section) as class,concat(s.firstname,' ',s.lastname) as name FROM students s inner join class c on c.id = s.class_id inner join user_form us on c.id = us.class_id order by concat(firstname,' ',lastname) asc ";
        $qry = $this->conn->query($sql);
        return $qry->fetch_all(MYSQLI_ASSOC);
    }
    public function get_student($id=""){
        $sql = "SELECT `s`.*, concat (`c`.`level`,`-`,`c`.`section`) as `class` FROM `students` inner join `class` on `s`.`class_id` = `c`.`id` where `s`.`id` = '{$id}'";
        $qry = $this->conn->query($sql);
        $result = $qry->fetch_assoc();
        return $result;
    }
    public function attendanceStudents($class_id = "", $class_date = ""){
        if(empty($class_id) || empty($class_date))
            return [];
        $sql = "SELECT `s`.*, COALESCE((SELECT `status` FROM `attendance` where `student_id` = `s`.id and `class_date` = '{$class_date}' ), 0) as `status` FROM `students` where `class_id` = '{$class_id}' order by `level` ASC,`section` ASC";
        $qry = $this->conn->query($sql);
    ...
    public function attendanceStudentsMonthly($class_id = "", $class_month = ""){
        if(empty($class_id) || empty($class_month))
            return [];
        $sql = "SELECT `s`.* FROM `students` where `class_id` = '{$class_id}' order by `level` ASC,`section` ASC";
        $qry = $this->conn->query($sql);
        $result = $qry->fetch_all(MYSQLI_ASSOC);
    ...
    public function save_attendance(){
        ...
        foreach($student_id as $k => $sid){
            ...
            $check = $this->conn->query("SELECT id FROM `attendance` where `student_id` = '{$sid}' and `class_date` = '{$class_date}'");
            if($check->num_rows > 0){
                ...
                try{
                    $update = $this->conn->query("UPDATE `attendance` set `status` = '{$stat}' where `id` = '{$att_id}'");
        ...
        {
            try{
                $sql =  $this->conn->query("INSERT INTO `attendance` ( `student_id`, `class_date`, `status` ) VALUES {$sql_values}");
            }catch(Exception $e){
                if(!empty($errors)) $errors .= "<br>";
                $errors .= $e->getMessage();
            }
