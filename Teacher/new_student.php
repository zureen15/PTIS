            ...
            <div class="form-group text-dark">
              <div class="form-group">
                ...
                <select name="class_id" id="" class="form-control select2 select2-sm" required>
                  <option></option> 
                  <?php 
                        $class = $conn->query("SELECT * FROM class c inner join user_form us on c.id = us.class_id order by level asc,section asc ");
                        while($row = $class->fetch_array()):
                  ?>
                        <option value="<?php echo $row['id'] ?>" <?php echo isset($class_id) == $row['id'] ? "selected" : '' ?>><?php echo ucwords($row['level'].'-'.$row['section']) ?></option>
                  <?php endwhile; ?>
                </select>
              </div>
            </div>
